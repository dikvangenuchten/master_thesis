...
\begin{table}[htp]
  ...
  \begin{center}
    \begin{tabular}{p{0.05\textwidth}p{0.25\textwidth}p{0.25\textwidth}p{0.25\textwidth}}
      ...
      2--4   & Setting up training pipeline using a very simple model.                                         & Reproducible training with proper metrics and results.                                                     & Code to train a simple generative object detection model, with trackable metrics (e.g. in WandB) \\
      5--9   & Improve the model                                                                               & A model which has reasonable results compared to (similar sized) SOTA models                               & more code                                                                                        \\
      9--10  & Preliminary experimentation                                                                     & Hopefully some promising results                                                                           &                                                                                                  \\
      11--14 & Fixing all problems created in earlier steps and encountered during preliminary experimentation & Code that is not broken anymore.                                                                           & Short experiments that show that the code is working as expected                                 \\
      15--18 & Writing report and running long experiments                                                     & Some results                                                                                               & nothing                                                                                          \\
      19     & Hand in draft-report to supervisors for final feedback                                          & Implementing the received feedback                                                                         & Final version of thesis                                                                          \\
...
\paragraph*{Possible data sharing restrictions} As this project is in collaboration with a company, iff company IP is used for (parts of) the research it might not be possible to publicize those parts of the code. However, as this is a research project and does not (necessarily) involve running the code on production units. It is likely that a large part of the code can be made public.
