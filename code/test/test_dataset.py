...
def test_toy_transform(test_image: Image):
    ...
    assert (torch.where(mask == 0, img, 0) == torch.where(mask == 0, test_image, 0)).all(), "The image was modified outside of the mask"
    assert (torch.where(mask == 1, img[0:1], 255) == 255).all(), "The red blob was not present"
