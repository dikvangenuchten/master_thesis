...
def main():
    ...
    print(df)
    df.style.highlight_min(props="textbf:--rwrap;",).format(precision=2).to_latex(
            "../thesis/figures/model_characteristics.tex",
...
def benchmark_single_model(model_name):
    with hydra.initialize("../src/conf/model"):
        ...
        if "skip_connections" in cfg:
            # During inference only the mean is required (i.e. projection)
            cfg["skip_connections"] = ["proj", "proj", "proj", "proj", "proj"]
        ...
        return {
            ...
            **{f"Inference Speed (ms),{r['batch_size']}": r["timer"].median * 1000 for r in results},
            **{f"Memory Usage (mb),{r['batch_size']}": r["peak memory"] * 1e-6 for r in results},
        }


def benchmark_inference(model, model_name: str, batch_size=1, input_shape=(3, 128, 128)):
    model = model.to("cuda")
    results = []
    for batch_size in [1,2,8,32]:
        ...
        with torch.inference_mode():
            result = t1.adaptive_autorange(0.0001)
        results.append({
