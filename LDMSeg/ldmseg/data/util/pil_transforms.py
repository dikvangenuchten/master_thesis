...
class RandomGaussianBlur(object):
    ...
    def __init__(self, sigma=[0.1, 2.0], p=0.2):
        self.sigma = sigma
        self.p = p
        ...
        if random.random() < 0.5:
            for elem in sample.keys():
                if elem in ["image", "image_semseg"]:
                    ...
                    sample[elem] = sample[elem].filter(
    ...
    def __call__(self, sample):
        for elem in sample.keys():
            if elem in [
                ...
                sample[elem] = self.crop_and_resize(
                    sample[elem], self.size[0], self.size[1], mode=INT_MODES[elem]
                )
        return sample
        ...
        for elem in sample.keys():
            if "meta" in elem or "text" in elem:
                ...
                sample[elem] = self.to_tensor(
                    sample[elem]
                )  # Regular ToTensor operation

            elif elem in ["semseg", "mask", "class_labels"]:
                sample[elem] = torch.from_numpy(
                    np.array(sample[elem])
                ).long()  # Torch Long
