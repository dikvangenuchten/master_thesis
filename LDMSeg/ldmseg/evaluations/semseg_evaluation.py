...
class SemsegMeter(object):
    def __init__(
        self, num_classes, class_names, has_bg=True, ignore_index=255, gpu_idx="cuda"
    ):
        ...
        if not suppress_prints or verbose:
            ...
            if verbose:
                for i_part in range(self.num_classes):
                    print(
                        "IoU class %s is %.2f"
                        % (self.class_names[i_part], 100 * jac[i_part])
                    )
    ...
    def synchronize_between_processes(self):
        ...
        if not is_dist_avail_and_initialized():
            return
        t = torch.tensor(
            [self.tp, self.fp, self.fn], dtype=torch.float64, device=self.gpu_idx
        )
        dist.barrier()
        dist.all_reduce(t)
        self.tp = t[0]
