...
# Hack to recognise ldmseg dir
sys.path.insert(0, "/workspaces/master_thesis/LDMSeg/")

from ldmseg.trainers import TrainerDiffusion
from ldmseg.models import (
    UNet,
    GeneralVAESeg,
    GeneralVAEImage,
    get_image_descriptor_model,
)
...
def main(cfg: DictConfig) -> None:
    ...
    cfg_dataset = (
        cfg_base | cfg_dataset
    )  # overwrite base configs with dataset specific configs
    root_dir = os.path.join(cfg["env"]["root_dir"], project_dir)
    data_dir = cfg["env"]["data_dir"]
    cfg_dataset, project_name = prepare_config(
        cfg_dataset, root_dir, data_dir, run_idx=cfg["run_idx"]
    )
    ...
    cfg_dist["distributed"] = (
        cfg_dist["world_size"] > 1 or cfg_dist["multiprocessing_distributed"]
    )
    # handle debug mode
    if cfg["debug"]:
    ...
    if cfg["debug"]:
        ...
        mp.spawn(
            ...
            args=(ngpus_per_node, cfg_dist, cfg_dataset, project_name),
        )
...
def main_worker(
    ...
    sys.stdout = Logger(
        os.path.join(p["output_dir"], f'log_file_gpu_{cfg_dist["gpu"]}.txt')
    )
    ...
    if p["shared_vae_encoder"]:
        semseg_vae_encoder = torch.nn.Sequential(
            vae_image.encoder, vae_image.quant_conv
        )  # NOTE: deepcopy is safer
    vae_semseg = GeneralVAESeg(**p["vae_model_kwargs"], encoder=semseg_vae_encoder)
    print(
        f"Scaling factors: image {vae_image.scaling_factor}, semseg {vae_semseg.scaling_factor}"
    )

    # load the pretrained UNet model
    unet = UNet.from_pretrained(
        pretrained_model_path, subfolder="unet", cache_dir=cache_dir
    )
    ...
    weight_dtype = (
        torch.float32 if train_params["weight_dtype"] == "float32" else torch.float16
    )
    ...
    if image_descriptor_model is not None:
        image_descriptor_model = image_descriptor_model.to(
            cfg_dist["gpu"], dtype=weight_dtype
        )
    if text_encoder is not None:
        text_encoder = text_encoder.to(cfg_dist["gpu"], dtype=weight_dtype)

    # define noise scheduler
    noise_scheduler = DDIMNoiseScheduler(
        **p["noise_scheduler_kwargs"], device=cfg_dist["gpu"]
    )
    ...
    if cfg_dist["distributed"]:
        ...
        print(
            colored(f"Batch size per process is {train_params['batch_size']}", "yellow")
        )
        print(
            colored(f"Workers per process is {train_params['num_workers']}", "yellow")
        )
    else:
    ...
    trainer.resume(
        load_vae=True
    )  # looks for model at run_idx (automatically determined)
    trainer.load(
        model_path=p["load_path"], load_vae=True
    )  # looks for model at load_path
